...
hweights <- data.frame(country = row.names(LifeCycleSavings), resid = huberFit$resid, weight = huberFit$w)
...
qplot(LifeCycleSavings$sr, olsFit$fitted.values)+geom_point(color="gray")+geom_point(aes(y=bisquareFit$fitted.values),color="blue")+ylab("fitted values")
